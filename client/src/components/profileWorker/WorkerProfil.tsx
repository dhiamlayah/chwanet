...
import UpdateData from '../updateData/UpdateData';
 ...
 const WorkerProfil = ({profilPicture,WorkerInformations}:any) => {
   ...
   const {
    ...
    const isThisMe = ()=>{
      if(user){return setIsMe(user._id ===WorkerInformations._id)}
   ...
   return ( 
      <div >
        ...
        <div className={!showUpdateDiv ?"Profilbackground opacity-100 " :"Profilbackground opacity-25  "} style={{zIndex:"12"}}>
        <div id="content">
          <div
          ...
          <div id="workerName" className='d-flex'>
           {isMe && <h1 className="px-2" onClick={()=>handleUpdate('name',[firstName,lastName])}>X</h1>}
        ...
        {number===1 &&  <WorkerDiscreption handleUpdate={handleUpdate} isMe={isMe} state={state} delegation={delegation} discreption={discreption} phone={phone} workName={workName}/>}
        </div>
      </div>
      );
 }
